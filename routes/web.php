...
use App\Http\Controllers\KursusHomecontroller;
...
Route::resource('dashboard', KursusHomeController::class)->middleware('auth');
Route::get('/resume-bab', [KursusHomeController::class, 'resumeLastOpenedBab'])->name('bab.resume');
...
Route::put('/updateProfile', [\App\Http\Controllers\ProfileController::class, 'updateProfile'])->name('update.profile');
Route::put('/feedback', [\App\Http\Controllers\ProfileController::class, 'feedback'])->name('update.feedback');

Route::group(['middleware' => 'auth'], function() {
   
    Route::get('test',[\App\Http\Controllers\TestController::class, 'index'])->name('client.test');
    Route::get('post_test',[\App\Http\Controllers\TestController::class, 'post'])->name('client.posttest');
    Route::post('test',[\App\Http\Controllers\TestController::class, 'store'])->name('client.test.store');
    Route::get('test/{show_id}',[\App\Http\Controllers\TestController::class, 'show'])->name('client.test.show');
    ...
    Route::group(['middleware' => 'isAdmin','prefix' => 'admin', 'as' => 'admin.'], function() {
        ...
        Route::resource('sub_modul', SubController::class);
        Route::resource('bab', BabController::class);
        Route::resource('feedback', feedController::class);
    });
